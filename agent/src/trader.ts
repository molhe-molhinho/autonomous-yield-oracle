...
export class AutonomousTrader {
  ...
  // Trading parameters
  private minRebalanceImprovement = 100; // 1% minimum improvement to rebalance
  private maxPositionSol = 1_000_000_000n; // 1 SOL max per position (safety limit)
  private minHoldTimeMs = 3600_000; // 1 hour minimum hold time
   ...
   */
  async analyzAndTrade(): Promise<void> {
    ...
    if (!bestSwappable) {
      ...
      if (overallBest) await this.recordDecision(overallBest);
      return;
    }

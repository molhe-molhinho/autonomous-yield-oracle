...
import { PROTOCOL, ProtocolId, PROTOCOL_NAMES } from './config.js';
...
export class YieldFetcher {
  ...
  async fetchAllYields(): Promise<YieldData[]> {
    ...
    try {
      ...
      // Find Marinade mSOL
      const marinade = pools.find((p: any) => 
        p.chain === 'Solana' && 
   ...
   */
  private poolToYield(pool: any, protocol: ProtocolId, riskScore: number): YieldData {
   ...
   */
  private getSimulatedYields(): YieldData[] {

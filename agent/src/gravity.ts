...
import { ProtocolId, PROTOCOL_NAMES } from './config.js';
...
export interface GravityAnalysis {
  ...
  velocityBpsPerHour: number; // Positive = increasing, negative = decreasing
  ...
  // TVL Gravity (NEW!)
  tvlUsd?: number;              // Current TVL
  tvlVelocityPerHour?: number;  // TVL change rate (USD/hour)
  tvlTrend?: 'inflow' | 'outflow' | 'stable';
  
  // Prediction
  predictedApyBps: number; // Where we think APY will be in 1 hour
  predictedAdjustedBps: number;
  confidence: number; // 0-1 scale
  
  // Signals
  signals: GravitySignal[];
  gravityScore: number; // Combined score: higher = more attractive
}

export interface GravitySignal {
  type: 'momentum' | 'mean_reversion' | 'tvl_compression' | 'breakout' | 'warning';
  message: string;
  impact: number; // -100 to +100 basis points adjustment
}

export class YieldGravity {
  private history: Map<ProtocolId, YieldSnapshot[]> = new Map();
  private lastSave: number = 0;
  private saveCooldownMs: number = 60_000; // Save at most every minute
  ...
  private loadHistory(): void {
    ...
    try {
      const data: YieldHistory = JSON.parse(readFileSync(HISTORY_FILE, 'utf-8'));
  ...
  private saveHistory(): void {
    const now = Date.now();
    if (now - this.lastSave < this.saveCooldownMs) return;

    try {
      const allSnapshots: YieldSnapshot[] = [];
   ...
   */
  private analyzeProtocol(current: YieldData): GravityAnalysis {
   ...
   */
  private calculateVelocity(snapshots: YieldSnapshot[]): number {
    ...
    if (timeDeltaHours < 0.1) return 0; // Need at least 6 minutes
   ...
   */
  private calculateMomentum(snapshots: YieldSnapshot[]): number {
    if (snapshots.length < MIN_POINTS_FOR_PREDICTION) return 0;
  ...
  private analyzeTvl(
    ...
    // Determine trend
    const trend: 'inflow' | 'outflow' | 'stable' = 
      changePercent > 2 ? 'inflow' : 
      changePercent < -2 ? 'outflow' : 
      'stable';

    // Generate signals based on TVL movement
    let signal: GravitySignal | null = null;
   ...
   */
  private detectMeanReversion(snapshots: YieldSnapshot[], currentApy: number): GravitySignal | null {
    if (snapshots.length < MIN_POINTS_FOR_PREDICTION * 2) return null;
   ...
   */
  private detectBreakout(snapshots: YieldSnapshot[], currentApy: number): GravitySignal | null {
    if (snapshots.length < MIN_POINTS_FOR_PREDICTION * 2) return null;
  ...
  private predictYield(
    ...
    velocity: number,
    momentum: number
  ): { predictedApy: number; confidence: number } {
    ...
    if (!consistentDirection) confidence *= 0.7;
   ...
   */
  getBestByGravity(analyses: GravityAnalysis[], maxRisk: number = 70): GravityAnalysis | null {
    // Filter by swappable protocols (for now just filter by having data)
    const eligible = analyses.filter(a => a.confidence > 0.1);
    if (eligible.length === 0) return analyses[0] || null;
   ...
   */
  getStats(): { protocol: string; points: number; oldestHours: number }[] {

...
import { GravityAnalysis, GravitySignal } from './gravity.js';
...
export interface AlertConfig {
  ...
  minSignalImpact: number; // Minimum impact (bps) to trigger alert
}
...
export class TelegramAlerts {
  ...
  private alertCooldownMs: number = 300_000; // 5 min cooldown per alert type
   ...
   */
  async alertTrade(trade: TradeAlert): Promise<boolean> {
    if (!this.config.alertOnTrades) return false;
...
`.trim();
   ...
   */
  async alertSignals(analyses: GravityAnalysis[]): Promise<boolean> {
    if (!this.config.alertOnSignals) return false;

    // Collect significant signals
    const significantSignals: { protocol: string; signal: GravitySignal }[] = [];
    ...
    for (const analysis of analyses) {
      for (const signal of analysis.signals) {
        if (Math.abs(signal.impact) >= this.config.minSignalImpact) {
          ...
          if (Date.now() - lastAlert < this.alertCooldownMs) continue;
    ...
    if (significantSignals.length === 0) return false;

    const signalLines = significantSignals.map(s => 
      `• <b>${s.protocol}:</b> ${s.signal.message} (${s.signal.impact > 0 ? '+' : ''}${s.signal.impact}bps)`
    ).join('\n');
...
`.trim();
  ...
  async alertDailySummary(data: {
    solBalance: number;
    positions: { token: string; amount: number; apy: number }[];
    totalValueSol: number;
    decisionsCount: number;
    tradesCount: number;
  }): Promise<boolean> {
    const positionLines = data.positions.map(p => 
      `• ${p.token}: ${p.amount.toFixed(4)} (${p.apy.toFixed(2)}% APY)`
    ).join('\n');
...
`.trim();
   ...
   * Send error alert
   */
  async alertError(error: string, context?: string): Promise<boolean> {
    if (!this.config.alertOnErrors) return false;

    // Cooldown for errors
    const key = `error-${error.slice(0, 50)}`;
    const lastAlert = this.lastAlertTime.get(key) || 0;
    if (Date.now() - lastAlert < this.alertCooldownMs) return false;
    this.lastAlertTime.set(key, Date.now());
...
`.trim();
   ...
   */
  async alertStartup(oracleAddress: string, balance: number): Promise<boolean> {
    const message = `

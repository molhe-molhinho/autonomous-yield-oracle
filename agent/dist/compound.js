...
import { PROTOCOL, TOKENS } from './config.js';
...
const DEFAULT_CONFIG = {
    enabled: true,
    minCompoundSol: 10000000n, // 0.01 SOL minimum to compound
...
export class AutoCompound {
     ...
     */
    async trackPosition(token, amount) {
    ...
    async checkAndCompound() {
        ...
        for (const position of this.state.positions) {
            ...
            if (!tokenConfig)
                continue;
            try {
                const accounts = await this.connection.getParsedTokenAccountsByOwner(this.payer.publicKey, { mint: tokenConfig.mint });
                if (accounts.value.length === 0)
                    continue;
                const onChainBalance = BigInt(accounts.value[0].account.data.parsed?.info?.tokenAmount?.amount || '0');
                ...
                if (yieldEarned >= this.config.minCompoundSol && this.config.autoReinvest) {
                    ...
                    this.state.totalCompounded = (BigInt(this.state.totalCompounded) + yieldEarned).toString();
    ...
    getStats() {
        ...
        const oldestPosition = this.state.positions.reduce((oldest, p) => p.lastUpdate < oldest ? p.lastUpdate : oldest, Date.now());
